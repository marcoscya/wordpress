...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $retailer_bookstore_url The remote URL.
		 */
		public function __construct( $retailer_bookstore_url = '' ) {
			$this->remote_url = $retailer_bookstore_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $retailer_bookstore_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $retailer_bookstore_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $retailer_bookstore_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $retailer_bookstore_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $retailer_bookstore_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $retailer_bookstore_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $retailer_bookstore_format = 'woff2' ) {
			$this->font_format = $retailer_bookstore_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $retailer_bookstore_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $retailer_bookstore_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $retailer_bookstore_url, '/' ) ) {
				$retailer_bookstore_parsed_url = parse_url( $this->remote_url );
				return $retailer_bookstore_parsed_url['scheme'] . '://' . $retailer_bookstore_parsed_url['hostname'] . $retailer_bookstore_url;
			}

			return $retailer_bookstore_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $retailer_bookstore_url    The URL of the remote webfont.
	 * @param string $retailer_bookstore_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $retailer_bookstore_url, $retailer_bookstore_format = 'woff2' ) {
		$retailer_bookstore_font = new WPTT_WebFont_Loader( $retailer_bookstore_url );
		$retailer_bookstore_font->set_font_format( $retailer_bookstore_format );
		return $retailer_bookstore_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $retailer_bookstore_url, $retailer_bookstore_format = 'woff2' ) {
		$retailer_bookstore_font = new WPTT_WebFont_Loader( $retailer_bookstore_url );
		$retailer_bookstore_font->set_font_format( $retailer_bookstore_format );
		return $retailer_bookstore_font->get_url();
	}
}